...
source(file.path("R", "load-data.R"))
list2env(load_datasets(), envir = globalenv())
```
...
cf <- instrumental_forest(
  X = as.matrix(X_train),
  Y = as.numeric(train$num_visit_cens_ed),
  W = as.numeric(train$medicaid),
  Z = as.numeric(train$treatment),
  honesty = TRUE
)
